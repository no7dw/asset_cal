...
Asset.prototype.cal = function(asset, time){
	...
	var calTime = date_util.getDate(null, 0, isSetZeroTime)|| time;

	if(asset.end_time <= calTime)//may undefined
	{
		console.log('$$$ time not right , may expired' , asset.end_time, calTime);
		return 0;
	}
	if(!asset.bonus_type || asset.bonus_type == 'ex_money'){
		...
		return assetDailyReturn  = (asset.rate) * (asset.amount|| asset.num) / 365;	
	}
